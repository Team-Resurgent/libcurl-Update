...
#error IPV6_requires_winsock2
  ...
  #ifdef _XBOX
// Xbox does not support LoadLibrary or GetProcAddress
#else
...
{
#ifdef _XBOX
  // Xbox does not support FreeLibrary
#else
  if(s_hIpHlpApiDll) {
...
HMODULE Curl_load_library(LPCTSTR filename) {
    (void)filename;
    return NULL;  // Xbox does not support dynamic library loading
} 
#else
  ...
  /* Detect if KB2533623 is installed, as LOAD_LIBARY_SEARCH_SYSTEM32 is only
    ...
    #ifdef _XBOX
	UINT systemdirlen = 0;  // Xbox does not have a system directory
	#else
